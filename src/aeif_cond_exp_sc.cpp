...
nest::RecordablesMap< stepcurrentmodule::aeif_cond_exp_sc > stepcurrentmodule::aeif_cond_exp_sc::recordablesMap_;
...
template <>
void
RecordablesMap< stepcurrentmodule::aeif_cond_exp_sc >::create()
{
  // use standard names whereever you can for consistency!
  insert_( nest::names::V_m, &stepcurrentmodule::aeif_cond_exp_sc::get_y_elem_< stepcurrentmodule::aeif_cond_exp_sc::State_::V_M > );
  insert_( nest::names::g_ex, &stepcurrentmodule::aeif_cond_exp_sc::get_y_elem_< stepcurrentmodule::aeif_cond_exp_sc::State_::G_EXC > );
  insert_( nest::names::g_in, &stepcurrentmodule::aeif_cond_exp_sc::get_y_elem_< stepcurrentmodule::aeif_cond_exp_sc::State_::G_INH > );
  insert_( nest::names::w, &stepcurrentmodule::aeif_cond_exp_sc::get_y_elem_< stepcurrentmodule::aeif_cond_exp_sc::State_::W > );
}
}


extern "C" int
stepcurrentmodule::aeif_cond_exp_sc_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef stepcurrentmodule::aeif_cond_exp_sc::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const stepcurrentmodule::aeif_cond_exp_sc& node = *( reinterpret_cast< stepcurrentmodule::aeif_cond_exp_sc* >( pnode ) );
  ...
  f[ S::V_M ] = is_refractory
    ? 0.
    : ( -node.P_.g_L * ( V - node.P_.E_L ) + I_spike - I_syn_exc - I_syn_inh - w + node.P_.I_e + node.B_.I_stim_ + node.B_.I_step_amp_ )
      / node.P_.C_m;
...
stepcurrentmodule::aeif_cond_exp_sc::Parameters_::Parameters_()
  : V_peak_( 0.0 )    // mV
  , V_reset_( -60.0 ) // mV
  , t_ref_( 0.0 )     // ms
  , g_L( 30.0 )       // nS
...
stepcurrentmodule::aeif_cond_exp_sc::State_::State_( const Parameters_& p )
  : r_( 0 )
{
  y_[ 0 ] = p.E_L;
...
stepcurrentmodule::aeif_cond_exp_sc::State_::State_( const State_& s )
  : r_( s.r_ )
{
...
stepcurrentmodule::aeif_cond_exp_sc::State_&
stepcurrentmodule::aeif_cond_exp_sc::State_::operator=( const State_& s )
{
  r_ = s.r_;
  for ( size_t i = 0; i < STATE_VEC_SIZE; ++i )
  {
...
void
stepcurrentmodule::aeif_cond_exp_sc::Parameters_::get( DictionaryDatum& d ) const
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::Parameters_::set( const DictionaryDatum& d, Buffers_& buffers, Node* node )
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::State_::get( DictionaryDatum& d ) const
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::State_::set( const DictionaryDatum& d, const Parameters_&, Node* node )
{
...
stepcurrentmodule::aeif_cond_exp_sc::Buffers_::Buffers_( aeif_cond_exp_sc& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
stepcurrentmodule::aeif_cond_exp_sc::Buffers_::Buffers_( const Buffers_&, aeif_cond_exp_sc& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
stepcurrentmodule::aeif_cond_exp_sc::aeif_cond_exp_sc()
  : ArchivingNode()
  , P_()
  , S_( P_ )
  , B_( *this )
...
stepcurrentmodule::aeif_cond_exp_sc::aeif_cond_exp_sc( const aeif_cond_exp_sc& n )
  : ArchivingNode( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
stepcurrentmodule::aeif_cond_exp_sc::init_buffers_()
{
  ...
  B_.sys_.function = aeif_cond_exp_sc_dynamics;
...
void
stepcurrentmodule::aeif_cond_exp_sc::pre_run_hook()
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::update( const Time& origin, const long from, const long to )
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::handle( SpikeEvent& e )
{
...
void
stepcurrentmodule::aeif_cond_exp_sc::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
stepcurrentmodule::aeif_cond_exp_sc::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
