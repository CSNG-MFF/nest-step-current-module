...
#ifndef IAF_COND_EXP_SC_NC_H
#define IAF_COND_EXP_SC_NC_H
 ...
 */
extern "C" int iaf_cond_exp_sc_nc_dynamics( double, const double*, double*, void* );
...
``iaf_cond_exp_sc_nc`` is an implementation of a spiking neuron using integrate-and-fire
dynamics with conductance-based synapses and includes a built-in step current generator
and an built-in noise generator.
...
class iaf_cond_exp_sc_nc : public nest::ArchivingNode
{

public:
  iaf_cond_exp_sc_nc();
  iaf_cond_exp_sc_nc( const iaf_cond_exp_sc_nc& );
  ~iaf_cond_exp_sc_nc();
...
private:
  ...
  friend int iaf_cond_exp_sc_nc_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class RecordablesMap< iaf_cond_exp_sc_nc >;
  friend class UniversalDataLogger< iaf_cond_exp_sc_nc >;
...
private:
  ...
  struct Buffers_
  {
    Buffers_( iaf_cond_exp_sc_nc& );                  //!< Sets buffer pointers to 0
    Buffers_( const Buffers_&, iaf_cond_exp_sc_nc& ); //!< Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< iaf_cond_exp_sc_nc > logger_;
  ...
  static RecordablesMap< iaf_cond_exp_sc_nc > recordablesMap_;
};


inline port
iaf_cond_exp_sc_nc::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
iaf_cond_exp_sc_nc::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_cond_exp_sc_nc::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
iaf_cond_exp_sc_nc::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_cond_exp_sc_nc::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
iaf_cond_exp_sc_nc::set_status( const DictionaryDatum& d )
{
...
inline Time
iaf_cond_exp_sc_nc::Parameters_::get_default_dt()
{
